...
def follow(request,user_id):
    user_follower = get_object_or_404(Follow, username = user_id)
    if request.method == "GET":
        if user_follower.follower.filter(username = request.user.username):
            user_follower.follower.remove(request.user)
        else:
            user_follower.follower.add(request.user)
    return JsonResponse({
        "message_follow": "success",
        "followers": user_follower.number_of_followers
    })
