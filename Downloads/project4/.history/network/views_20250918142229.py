...
def following_post(request, username):
    recent_user = get_object_or_404(User, username = username)
    user_followings = recent_user.following.all()
    view_posts = Post.objects.filter(username = user_followings.username).order_by('-timestamp')
    return render(request, 'followingpost.html',{
        "view_posts": view_post
    })
