...
def following_post(request, username):
    ...
    user_followings = recent_user.following.username() # lấy những người mà user hiện tại đang follow
    view_posts = []
    for user_following in user_followings:
        view_post = Post.objects.filter(username = user_following).order_by('-timestamp')
