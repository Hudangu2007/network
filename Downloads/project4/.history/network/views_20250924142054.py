...
def savepost(request, id):
    if request.method == 'POST':
        ...
        data = json.loads(request.body) #phải lấy thông tin từ json trước mới được
        newcontent = data.get('newcontent') #xong lấy thông tin cụ thể là thằng newcontent
        currentpost.content = newcontent
        currentpost.save()
        #return render(request,"network/profile.html")
        #đang làm việc với json thì return json ko phải django
        return JsonResponse({
                'status': 'success',
