...
def following_post(request, username):
    ...
    user_followings = recent_user.following.following_list() # lấy những người mà user hiện tại đang follow
    view_posts = []
    for user_following in user_followings:
        view_post = Post.objects.filter(username = user_following.username).order_by('-timestamp')
        view_posts.append(view_post)
    return render(request, 'followingpost.html',{
